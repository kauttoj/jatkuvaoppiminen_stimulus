...
function poliTimelineGen (stims, prompts, peers, opts) {
    ...
    for(var i = 0; i < stims.length; i++) {
      ...
      var block = {
	    ...
	    data			: {block_num: block_num},
	    timeline: [
		{prompt: prompts[i] + "<br><span id='subprompt'>Miten <u>sinä</u> valitset?</span>", peer: 0},
		{peer: 1, prompt: prompts[i] + "<br><span id='subprompt'>Miten arvelet että <u>${peer}</u> valitsi?</span>"},
		{peer: 2, prompt: prompts[i] + "<br><span id='subprompt'>Miten arvelet että <u>${peer}</u> valitsi?</span>"},
		{peer: 3, prompt: prompts[i] + "<br><span id='subprompt'>Miten arvelet että <u>${peer}</u> valitsi?</span>"},
	    ]
	}
	if (i == 0) {
	    block.timeline[0].phase = "FIRST";
	...
	if (i == stims.length - 1) {
	    block.timeline.push({phase: "MYSTERY", prompt:'${peer1} ja ${peer2} valitsivat seuraavasti'});
	}
	$.extend(block, opts.timing);
	timeline.push(block);
    }
